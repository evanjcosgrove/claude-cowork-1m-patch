...
- **Two same-length JS swaps** in the model-resolution function (minified name rotates: `ZAt`, `A7e`, etc.). Both must preserve byte length so V8's compiled bytecode cache stays valid.
  ...
  - **1b — TWO known forms**, detected at runtime by the preflight:
    - **Form A (regex)** for older asars (~v1.5xx and earlier): `sonnet-4-6|opus-4-6` → `opus-4-[67](?:)(?:)` (19 bytes). `(?:)(?:)` are zero-width non-capturing groups serving as 8 bytes of padding.
    - **Form B (array)** for newer asars (~v1.31xx+): `["claude-sonnet-4-6","claude-opus-4-6"]` → `[ "claude-opus-4-6","claude-opus-4-7" ]` (39 bytes). The newer asar swapped the regex literal for a JS array used with `.some(t => e.includes(t))` (substring match). Same-length swap; sonnet intentionally dropped per "Opus-only scope" caveat.
...
- **Layer 1b form rotation is its own non-obvious failure mode.** When Anthropic refactored the model gate from regex to array (~v1.31xx), older patch scripts that only knew the regex anchor would detect `needs_1a` only, half-patch, and fail verification — leaving the asar with Layer 1a applied but Layer 1b untouched. The current preflight refuses to proceed in this case (`unknown` state). When Anthropic refactors the gate again, expect the same failure mode and add a Form C alongside A/B in the preflight + patch-dispatch.
