...
*Since this writeup,* two more regressions have landed. On April 18, `opus-4-7` shipped to Cowork and broke a second JS gate in the same function - a model-allow-list I'd glossed over because the original regex already matched what I needed. On April 20, v1.3109 refactored that gate from a regex literal into a JS array. Different syntax, same three OR conditions, same flag ID, same `[1m]` template. The byte-anchor approach holds because the strings it pins on - the flag ID, the model names - are the stable parts. Expect the surrounding syntax to keep rotating; each iteration is tracked in [CHANGELOG.md](https://github.com/evanjcosgrove/claude-cowork-1m-patch/blob/main/CHANGELOG.md) and [docs/root-cause-analysis.md](https://github.com/evanjcosgrove/claude-cowork-1m-patch/blob/main/docs/root-cause-analysis.md).
