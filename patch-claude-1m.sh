  ...
  if [ "$exit_code" -ne 0 ] \
    ...
    if [ "${PATCH_RESTORE_ON_FAIL:-0}" = "1" ]; then
      ...
      if cp "$BACKUP_PATH" "$ASAR_PATH" 2>/dev/null \
        ...
        echo "Restored. Re-launch Claude with:"
        echo "  osascript -e 'quit app \"Claude\"'; sleep 3; open -a Claude"
      else
...
echo ""
echo "Next steps:"
echo "  1. Relaunch Claude Desktop with:"
echo "     osascript -e 'quit app \"Claude\"'; sleep 3; open -a Claude"
echo "  2. Start a NEW Cowork session (existing sessions keep old model)"
echo "  3. Verify: model should show 'Opus 4.6 Extended', 'Opus 4.7 Extended', or similar"
echo "     (or grep ~/Library/Logs/Claude/cowork_vm_node.log for 'opus-4-7[1m]')"
echo "     Manual reopen usually works too, but if the first reopen stutters,"
echo "     freezes, or crashes, quit Claude and use the command above."
echo ""
echo "Rollback:"
echo "  cp '$BACKUP_PATH' '$ASAR_PATH'"
echo "  cp '$PLIST_BACKUP_PATH' '$PLIST_PATH'"
